...
export function createUiCopy(overrides: UiCopyOverrides): UiCopy {
  const {
    deleteHistory,
    deleteFavorite,
    deleteHistoryPrefix,
...
const UI_COPY: Partial<Record<DisplayLanguage, UiCopy>> = {
  ...PRIMARY_UI_COPY,
};

let additionalUiCopyPromise: Promise<void> | null = null;

export async function loadUiCopy(language: DisplayLanguage): Promise<void> {
  if (
    language === 'zh' ||
    language === 'en' ||
    language === 'ja' ||
  ...
  additionalUiCopyPromise ??= import('./ui-additional').then(
    ({ ADDITIONAL_UI_COPY }) => {
      Object.assign(UI_COPY, ADDITIONAL_UI_COPY);
    },
  );

  await additionalUiCopyPromise;
}
...
export function getUiCopy(language: DisplayLanguage): UiCopy {
  return UI_COPY[language] ?? PRIMARY_UI_COPY.en;
}
